...
@app.callback([Output('live-graph', 'figure'),
               Output('boost-display', 'children')],
              [Input('interval-component', 'n_intervals')])
def update_graph(n_intervals):
    new_data = data.iloc[:n_intervals+1]

    figure = {
        'data': [
            go.Scatter(x=new_data['Time (sec)'], y=new_data['Engine Power (hp)'],
                       mode='lines+markers', name='Engine Power (hp)'),
            go.Scatter(x=new_data['Time (sec)'], y=new_data['Engine Torque (N•m)'],
                       mode='lines+markers', name='Engine Torque (N•m)'),
        ],
        'layout': {
            'title': 'Dyno Chart',
            'xaxis': {'title': 'Time (sec)'},
            'yaxis': {'title': 'HP', 'side': 'left', 'range': [0, 280]},
            'yaxis2': {'title': 'N•m', 'overlaying': 'y', 'side': 'right', 'range': [0, 560]},
            'showlegend': True
        }
    }

    latest_boost = new_data['Boost (bar)'].iloc[-1]

    boost_display = f'Boost: {latest_boost} bar'

    return figure, boost_display
...
#     plt.savefig(f"{folder}/uphill.png", bbox_inches='tight')
...
#     label_pullt(0.02, 0.98, label_text, transform=ax2.transAxes, ha='left', va='top', fontsize=12)
